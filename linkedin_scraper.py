...
def linkedin_companies_parser(url):
    ...
    print "Fetching :",url
    proxies= {"http": "http://df43879a79fe7f9e122068928b86823d: @workdistribute.charityengine.com:20000", "https":"http://df43879a79fe7f9e122068928b86823d: @workdistribute.charityengine.com:20000"}
    response = requests.get(url, headers = headers,proxies = proxies,verify=False)
    formatted_response = response.content.replace('<!--', '').replace('-->', '')
    doc = html.fromstring(formatted_response)
    datafrom_xpath = doc.xpath('//code[@id="stream-promo-top-bar-embed-id-content"]//text()')
    # datafrom_xpath = doc.xpath('//code[@id="stream-about-section-embed-id-content"]')
